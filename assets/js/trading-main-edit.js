...
var edit_strategy = JSON.parse(localStorage.getItem("edit-strategy"));
$("#strat-name").val(edit_strategy.name);
$("#strat-desc").val(edit_strategy.description);
// if($("#adx-ok").is(":checked"))  {total_adx=parseInt($("#adx-slider").val(strategies[0].adx_wgth))}
// if($("#rsi-ok").is(":checked"))  {total_rsi=parseInt($("#rsi-slider").val(strategies[0].rsi_wgth))}
// if($("#srsi-ok").is(":checked")) {total_srsi=parseInt($("#srsi-slider").val(strategies[0].srsi_wgth))}
// if($("#mom-ok").is(":checked"))  {total_mom=parseInt($("#mom-slider").val(strategies[0].mom_wgth))}
...
function editStrategy() {
    ...
    if(total_pctg===100&&strat_name.length>0&&strat_name.length<=10&&strat_desc.length>0&&strat_desc.length<=50){
        ...
        var edited_strategy={  name: strat_name ,
                            desc: strat_desc,
                            adx_wgth: total_adx,
                            rsi_wgth: total_rsi,
        ...
        all_strategies.push(edited_strategy);
